...
export default function CommunityModelFlow({ initialModelId }: CommunityModelFlowProps) {
        ...
        try {
          ...
          if (fetchedModelData) {
            setModelData({
              ...
              activeConstitutionId: fetchedModelData.activeConstitutionId || undefined,
  ...
  const handleSaveAbout = async (data: AboutZoneData) => {
    ...
    try {
      ...
      setModelData(prevData => ({ ...prevData, ...data } as ExtendedAboutZoneData));
      setShowToast(true);
      if (!isExistingModel) {
  ...
  const handleUpdatePrinciples = async (data: Partial<ExtendedAboutZoneData>) => {
    if (modelId) {
      try {
        ...
        setTimeout(() => setSavingStatus(prev => ({ ...prev, principles: 'idle' })), 2000);
      } catch (error) {
  ...
  const saveModelData = async (data: Partial<ExtendedAboutZoneData>, zone: string) => {
    if (!modelId) return;
    ...
    try {
      ...
      setTimeout(() => setSavingStatus(prev => ({ ...prev, [zone]: 'idle' })), 2000);
    } catch (error) {
      console.error('Error saving community model:', error);
          ...
          <>
            <div ref={zoneRefs.principles} id="principles">
              <PrinciplesZone 
                ...
                onComplete={() => !isExistingModel && setActiveZones(prev => [...prev, 'poll'])}
            ...
            <div ref={zoneRefs.poll} id="poll">
              <PollZone 
                ...
                onComplete={() => !isExistingModel && setActiveZones(prev => [...prev, 'communityModel'])}
                modelId={modelId || initialModelId!}
                modelData={{
                  name: modelData?.name || '',
                  bio: modelData?.bio || '',
                ...
                pollData={modelData?.polls?.[0] as (Poll & { statements: Statement[] }) | undefined}
                isExistingModel={isExistingModel}
                  ...
                  if (modelId) {
                    try {
                      ...
                      setModelData(prevData => {
                        ...
                        return {
                          ...prevData,
                          polls: [updatedPollData as Poll, ...(prevData.polls?.slice(1) || [])]
                        } as ExtendedAboutZoneData;
                      });
                      setSavingStatus(prev => ({ ...prev, poll: 'saved' }));
                      setTimeout(() => setSavingStatus(prev => ({ ...prev, poll: 'idle' })), 2000);
                    } catch (error) {
                ...
                onUpdate={(data) => {
                  setModelData(prevData => {
                    ...
                    return {
                      ...
                      constitutions: data.constitutions || prevData.constitutions || [],
                      activeConstitutionId: data.activeConstitutionId || prevData.activeConstitutionId
                    };
                  });
                  if (isExistingModel) {
                    ...
                    if (constitutions || activeConstitutionId !== undefined) {
                      debouncedSaveModelData({ constitutions, activeConstitutionId }, 'communityModel');
                    }
                  }
                }}
