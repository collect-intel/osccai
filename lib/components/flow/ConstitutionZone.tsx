...
export default function ConstitutionZone({ 
  ...
  // Sort constitutions in reverse order (newest first)
  const sortedConstitutions = [...modelData.constitutions].sort((a, b) => 
    b.version - a.version
  );

  const [selectedConstitution, setSelectedConstitution] = useState<Constitution | null>(
    sortedConstitutions[0] || null
  );
  const [isGenerating, setIsGenerating] = useState(false);
  const [isChatModalOpen, setIsChatModalOpen] = useState(false);
  const [constitutionToDelete, setConstitutionToDelete] = useState<Constitution | null>(null);
  ...
  return (
    <ZoneWrapper title="Constitutions" isActive={isActive} onToggle={onToggle} savingStatus={savingStatus}>
      <div className="flex bg-teal text-white rounded-lg overflow-hidden min-h-[200px]">
        <div className="w-1/3 p-4">
          ...
          <p className="text-sm mb-4">
            Generating a new constitution will use the community model bio and goals, and the latest poll-derived consensus principles.
          </p>
            ...
            {sortedConstitutions.map((constitution) => (
              <li key={constitution.uid} className="flex items-center">
                <button 
                  ...
                  `}
                >
                  <div>Constitution v{constitution.version} {constitution.uid === sortedConstitutions[0].uid && 
                    <span className="text-xs font-semibold ml-2">(latest)</span>}</div>
                  <div className="text-xs text-teal-200">
                    {formatDistanceToNow(new Date(constitution.createdAt), { addSuffix: true })}
                  </div>
      ...
      {isChatModalOpen && selectedConstitution && (
        <Modal isOpen={isChatModalOpen} onClose={() => setIsChatModalOpen(false)} minHeight={500}>
          <div className="w-full h-full max-w-4xl">
            <ConstitutionalAIChat
              ...
              initialMessages={[
                { role: 'assistant', content: `Hi there. You're chatting with an AI encoded with the constitution of your community model.` }
              ]}
            />
          </div>
        </Modal>
      ...
      {constitutionToDelete && (
        <Modal isOpen={!!constitutionToDelete} onClose={() => setConstitutionToDelete(null)}>
          <div className="bg-white rounded-lg">
            <h3 className="text-xl font-semibold mb-4">Confirm Deletion</h3>
            <p className="mb-6">Are you sure you want to delete Constitution v{constitutionToDelete.version}?</p>
