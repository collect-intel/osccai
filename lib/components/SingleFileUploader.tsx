...
interface SingleFileUploaderProps {
  ...
  renderPreview: (file: { name: string; url: string }, onReplace: () => void) => React.ReactNode;
}
...
const SingleFileUploader: React.FC<SingleFileUploaderProps> = ({
  ...
  const [file, setFile] = useState<{ name: string; url: string } | null>(initialFile || null);
  const [isUploading, setIsUploading] = useState(false);
  const [isDragging, setIsDragging] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const handleFileUpload = useCallback(async (uploadFile: File) => {
    if (uploadFile.size > maxFileSize || (validTypes.length && !validTypes.includes(uploadFile.type))) {
