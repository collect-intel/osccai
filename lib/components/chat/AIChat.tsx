...
interface AIChatProps {
  ...
  genStream: (messages: MessageWithFields[]) => Promise<AsyncGenerator<string | { [key: string]: string }, void, unknown>>;
}

export interface AIChatHandle {
  addMessage: (role: 'user' | 'assistant', content: string, submit?: boolean) => Promise<MessageWithFields[]>;
...
const AIChat = forwardRef<AIChatHandle, AIChatProps>(({
  ...
  const [messages, setMessages] = useState<MessageWithFields[]>(initialMessages);
  ...
  const submitMessages = useCallback(async (messages: MessageWithFields[]) => {
    ...
    try {
      ...
      currentMessageRef.current = { role: 'assistant', content: '', isStreaming: true };
      ...
      console.log('[AIChat] final currentMessage', currentMessageRef.current);
      ...
      if (currentMessageRef.current) {
        ...
        setMessages(prevMessages => prevMessages.map(msg => 
          msg === currentMessageRef.current ? { ...currentMessageRef.current } : msg
        ));
      }
  ...
  useImperativeHandle(ref, () => ({
    addMessage: async (role: 'user' | 'assistant', content: string, submit = false) => {
  ...
  const renderMessages = useCallback(() => {
    console.log('[AIChat] renderMessages', messages, currentMessageRef.current);
